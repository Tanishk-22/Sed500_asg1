...
void AnalogCircuit::run() {
	...
	glBegin(GL_LINES);
	glVertex2f(0.0, scalingFactor * windowHeight / 2.0);
	glVertex2f(windowWidth, scalingFactor * windowHeight / 2.0);
	glEnd();
	...
	glVertex2f(windowWidth / 2.0, windowHeight);
	glEnd();

	//Horizontal line markers
	for (int i = -5; i <= 5; ++i) {
		// Calculate the position of the marker on the line
		double xPos = windowWidth / 2.0 + i * (windowWidth / 10.0);

		// Draw the marker as a short vertical line
		glBegin(GL_LINES);
		glVertex2f(xPos, windowHeight / 2.0 - 10);
		glVertex2f(xPos, windowHeight / 2.0 + 10);
		glEnd();
	}
	...
	for (int i = -5; i <= 5; ++i) {
		// Calculate the position of the marker on the line
		double yPos = windowHeight / 2.0 + i * (windowHeight / 10.0);

		// Draw the marker as a short horizontal line
		glBegin(GL_LINES);
		glVertex2f(windowWidth / 2.0 - 10, yPos);
		glVertex2f(windowWidth / 2.0 + 10, yPos);
		glEnd();
	}
	...
	for (double time = 0.0; time < 0.6 * timeMax; time += T) {
		...
		for (auto& comp : component) {
			double componentVoltage = comp->GetVoltage(I, T);
			comp->Display();
			comp->Update();
		}
		CostFunctionV(I, V);
